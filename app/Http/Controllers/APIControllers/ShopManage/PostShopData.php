     ...
     function postshopdata() {
        ...
        try{
            ...
            if(  !$this->InsertShopData($inputData, $sd_id, $latitude, $longitude)
               ||!$this->InsertShopDataXref($stx_tag_type, $stx_tag_id, $sd_id)
               ||!$this->UpdateMemberData($md_clienttype, $md_id)
               ||!$memService->modify_member_clienttype('', $sat, $md_clienttype, $messageCode)
               ||!$this->InsertDepositBuyItmEreData($dbir_id, $dbir_expiredate, $md_id, $sd_id, $inputData , $cosdata)
               ||!$this->InsertSmbBind($sd_id, $md_id, $dbir_expiredate, $dbir_id, $smb_shoptype, $inputData['smb_bind_mail'], $smb_mail_sentid)
               ||!$this->Send_Email($inputData['smb_bind_mail'], $smb_mail_sentid, Hash('sha256',$sd_id.$md_id.$smb_mail_sentid))) {
