...
class ShopQueryList {
    function shopquerylist() {

        $functionName = 'shopquerylist';
        ...
        try {
            ...
            if (!ShopQueryList::CheckInput($inputData)) {
                $messageCode = '999999995';
                throw new \Exception($messageCode);
            }
            //檢查身份模組驗證
            ...
            $resultData['shoplistarray'] = ShopQueryList::GetShopListData($inputData, $messageCode);
    ...
    function GetShopListData($inputData, &$messageCode) {
        try {
            if($inputData['operation_type'] == 0 ) {
              ...
              $querydata = \App\Models\ICR_ShopData::GetShopDataList($inputData['spm_serno'], $inputData['sd_zipcode'], $inputData['sd_shopname'], $inputData['startid'], $inputData['queryamount']) ;
              ...
              $querydata = \App\Models\ICR_ShopData::GetShopDataList(null, null, $inputData['sd_shopname'], $inputData['startid'], $inputData['queryamount']) ;
            } else {
              //傳入值格式內容格式有誤，請重新輸入
