...
class ShopCouponReservationQuery {

    function shopcouponreservationquery() {
        $functionName = 'shopcouponreservationquery';
        ...
        try {
            ...
            //檢查輸入值
            if (!ShopCouponReservationQuery::CheckInput($inputData)) {
                $messageCode = '999999995';
                throw new \Exception($messageCode);
            }
            $md_id = null;
            //檢查Token、DeviceCode111
            ...
            //取得資料
            if (!ShopCouponReservationQuery::Get_ReservationData($inputData['sd_id'], $inputData['query_startdate'], $inputData['query_reply_status'], $reservationInfoarray, $last_update_date, $messageCode)) {
                throw new \Exception($messageCode);
            }
    ...
    function CheckInput(&$value) {
        ...
        if (!\App\Library\Commontools::CheckRequestArrayValue($value, 'query_startdate', 0, true, true)) {
            return false;
        }
        if (!\App\Library\Commontools::CheckRequestArrayValue($value, 'query_reply_status', 0, true, true)) {
            return false;
        }
        return true;
    }
     ...
     * @return boolean
     */
    function Get_ReservationData($sd_id, $query_startdate, $query_reply_status, &$reservationInfoarray, &$last_update_date, &$messageCode) {
        try {
            //$coupondata = \App\Models\ICR_ShopCouponData_m::Query_HaveReservation($sd_id, $lastupdate);
            $coupondata = \App\Models\ICR_ShopCouponData_m::Query_FiveDayReservation($sd_id, $query_startdate, $query_reply_status);//($sd_id);
