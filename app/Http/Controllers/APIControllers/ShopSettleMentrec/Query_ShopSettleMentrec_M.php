...
class Query_ShopSettleMentrec_M {
   function query_shopsettlementrec_m() {
        $functionName = 'query_shopsettlementrec_m';
        ...
        try{
            ...
            $resultData['shopsettlementrec_m'] = $this->createResultData($inputData['ssrm_id'],$inputData['sd_id']);
    ...
    function createResultData($ssrm_id,$sd_id) {
        try {
            ...
            if( $SsrmData[0]['ssrm_settlementpayment'] != 0) {
                switch ($SsrmData[0]['ssrm_settlementpayment'] ) {
                    case 1:
                        $SsrmData[0]['settlement_status'] = '已出款、調帳中，暫不出款';
                        break;
                     default:
                        break;
                }
            } else if( $SsrmData[0]['ssrm_settlementreview'] != 0){
                switch ($SsrmData[0]['ssrm_settlementreview']) {
                    case 1:
                        $SsrmData[0]['settlement_status'] = '已覆核';
                        break;
                    case 2:
                        $SsrmData[0]['settlement_status'] = '帳務有誤';
                        break;
                    case 3:
                        $SsrmData[0]['settlement_status'] = '已覆核﹙系統﹚';
                        break;
                    default:
                        break;
                }
            } /*else {
                switch ($ssrmData[0]['ssrm_settlementcomplete'] ) {
                    case 0:
                        $ssrmData[0]['settlement_status'] = '未完成';
                        break;
                     case 1:
                        $ssrmData[0]['settlement_status'] = '完成';
                        break;
                     default:
                        break;
                }
