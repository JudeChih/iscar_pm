...
window.Dom7 = function() {
		...
		function a(a) {
			...
			var s = a.url.indexOf("?") >= 0 ? "&" : "?",
				o = a.method.toUpperCase();
			...
			if ("json" === a.dataType && a.url.indexOf("callback=") >= 0) {
				var p, d = "f7jsonp_" + (Date.now() + (g += 1)),
					c = a.url.split("callback="),
					m = c[0] + "callback=" + d;
				if (c[1].indexOf("&") >= 0) {
				...
				return v.type = "text/javascript", v.onerror = function() {
					clearTimeout(p), t(void 0, void 0, "error", null, "scripterror"), t("ajaxComplete ajax:complete", {
						scripterror: !0
			...
			var w = null;
			if (("POST" === o || "PUT" === o || "PATCH" === o) && a.data)
				if (a.processData) {
					var C = [ArrayBuffer, Blob, Document, FormData];
					if (C.indexOf(a.data.constructor) >= 0) w = a.data;
					else {
			...
			return b.onload = function(e) {
				if (P && clearTimeout(P), b.status >= 200 && b.status < 300 || 0 === b.status) {
					...
					if ("json" === a.dataType) try {
						r = JSON.parse(b.responseText), t("ajaxSuccess ajax:success", {
							xhr: b
						}, "success", r, b.status, b)
					} catch (e) {
						t("ajaxError ajax:error", {
							xhr: b,
							parseerror: !0
						}, "error", b, "parseerror")
					} else r = "text" === b.responseType || "" === b.responseType ? b.responseText : b.response, t("ajaxSuccess ajax:success", {
						xhr: b
					}, "success", r, b.status, b)
				} else t("ajaxError ajax:error", {
					xhr: b
				}, "error", b, b.status);
				a.statusCode && (i.statusCode && i.statusCode[b.status] && i.statusCode[b.status](b), a.statusCode[b.status] && a.statusCode[b.status](b)), t("ajaxComplete ajax:complete", {
					xhr: b
				}, "complete", b, b.status)
			}, b.onerror = function(e) {
				P && clearTimeout(P), t("ajaxError ajax:error", {
					xhr: b
				}, "error", b, b.status), t("ajaxComplete ajax:complete", {
					xhr: b,
					error: !0
				}, "complete", b, "error")
			}, t("ajaxStart ajax:start", {
				xhr: b
			}, "start", b), t(void 0, void 0, "beforeSend", b), a.timeout > 0 && (b.onabort = function() {
				P && clearTimeout(P)
			}, P = setTimeout(function() {
				b.abort(), t("ajaxError ajax:error", {
					xhr: b,
					timeout: !0
				}, "error", b, "timeout"), t("ajaxComplete ajax:complete", {
					xhr: b,
					timeout: !0
				}, "complete", b, "timeout")
		...
		for (var l = function(e) {
				...
				base: "z",
				letters: "zⓩｚźẑżžẓẕƶȥɀⱬꝣ"
			}], d = {}, c = 0; c < p.length; c += 1)
			for (var m = p[c].letters, u = 0; u < m.length; u += 1) d[m[u]] = p[c].base;
		var h = {
			...
			each: function(e, a) {
				if ("object" == typeof e && a)
					if (Array.isArray(e) || e instanceof l) {
		...
		for (var g = 0, v = {
				scrollTo: function(e, a, t, r, n) {
					return void 0 === r && (r = "swing"), 4 === arguments.length && "function" == typeof r && (n = r, r = void 0), this.each(function() {
						function i(e) {
							...
							var a, l = Math.max(Math.min((e - b) / t, 1), 0),
						...
						for (var p = 0; p < this.length; p += 1) {
							...
							if (r) {
								if (d.dom7LiveListeners)
									for (var c = 0; c < d.dom7LiveListeners.length; c += 1) n ? d.dom7LiveListeners[c].listener === n && d.removeEventListener(o[l], d.dom7LiveListeners[c].proxyListener, s) : d.dom7Listeners[c].type === o[l] && d.removeEventListener(o[l], d.dom7LiveListeners[c].proxyListener, s)
				...
				parents: function(a) {
					for (var t = this, r = [], n = 0; n < this.length; n += 1)
						for (var i = t[n].parentNode; i;) a ? e(i).is(a) && r.push(i) : r.push(i), i = i.parentNode;
					return e(h.unique(r))
				},
				closest: function(e) {
					var a = this;
				...
				children: function(a) {
					for (var t = this, r = [], n = 0; n < this.length; n += 1)
						for (var i = t[n].childNodes, s = 0; s < i.length; s += 1) a ? 1 === i[s].nodeType && e(i[s]).is(a) && r.push(i[s]) : 1 === i[s].nodeType && r.push(i[s]);
		...
		function r(e) {
			...
			for (t = 0; t < n.length; t += 1) {
				...
				if (0 === t) i.push(s);
					...
					if (p = 0 === s.indexOf('"') ? l : o, d = 0 === s.indexOf('"') ? '"' : "'", 2 === s.match(p).length) i.push(s);
					else {
						for (a = 0, r = t + 1; r < n.length; r += 1)
							if (s += " " + n[r], n[r].indexOf(d) >= 0) {
								a = r, i.push(s);
								break
							}
						a && (t = a)
				...
				function r(e, o) {
					...
					for (u = 0; u < d.length; u += 1) {
						...
						if ("plain" !== h.type) {
							var f = void 0,
								g = void 0;
							if ("variable" === h.type && (f = a(h.contextName, c), m += "r += c(" + f + ", " + c + ");"), "helper" === h.type)
								if (h.helperName in i.helpers) g = t(h.contextName, c), m += "r += (Template7.helpers." + h.helperName + ").call(" + c + ", " + (g && g + ", ") + "{hash:" + JSON.stringify(h.hash) + ", data: data || {}, fn: " + l(h, o + 1) + ", inverse: " + p(h, o + 1) + ", root: root});";
								else {
		...
		window.Framework7 = function(e) {
			...
			var o = function(e, a) {
				...
				if (o.handleTouchStart = function(e) {
						...
						if (E = !1, O = !1, 0 === w) return i([L[0], z[0]]).transform(""), void(P && (y.transform("").css({
							opacity: ""
						}), x.transform("").css({
							opacity: ""
						...
						var a = (new Date).getTime() - C,
							t = !1;
						(a < 300 && w > 10 || a >= 300 && w > b / 2) && (L.removeClass("page-on-center").addClass("page-on-right"), z.removeClass("page-on-left").addClass("page-on-center"), P && (N.removeClass("navbar-on-center").addClass("navbar-on-right"), A.removeClass("navbar-on-left").addClass("navbar-on-center")), t = !0), i([L[0], z[0]]).transform("").addClass("page-transitioning"), P && (y.css({
							opacity: ""
						}).each(function() {
						...
						L.transitionEnd(function() {
							i([L[0], z[0]]).removeClass("page-transitioning"), P && (y.removeClass("page-transitioning").css({
								opacity: ""
			...
			r.triggerPageCallbacks = function(e, a, t) {
				...
				if (e.params.uniqueHistory) {
					...
					if (e.params.uniqueHistoryIgnoreGetParameters)
						for (D = [], L = h.split("?")[0], l = 0; l < e.history.length; l++) D.push(e.history[l].split("?")[0]);
			...
			r.messages = function(e, a) {
				...
				function t(e) {
					if (s) {
						...
						var D;
						if (u.length > 0)
							for (O = v / g, v < -g && (v = -g - Math.pow(-v - g, .8), k.length > 0 && (S = !0)), n = 0; n < y.length; n++) void 0 === y[n]._buttonOffset && (y[n]._buttonOffset = y[n].offsetLeft), p = y[n]._buttonOffset, D = i(y[n]), k.length > 0 && D.hasClass("swipeout-overswipe") && (D.css({
								left: (S ? -p : 0) + "px"
							}), S ? D.addClass("swipeout-overswipe-active") : D.removeClass("swipeout-overswipe-active")), D.transform("translate3d(" + (v - p * (1 + Math.max(O, -1))) + "px,0,0)");
						if (h.length > 0)
							for (O = v / f, v > f && (v = f + Math.pow(v - f, .8), T.length > 0 && (P = !0)), n = 0; n < C.length; n++) void 0 === C[n]._buttonOffset && (C[n]._buttonOffset = f - C[n].offsetLeft - C[n].offsetWidth), p = C[n]._buttonOffset, D = i(C[n]), T.length > 0 && D.hasClass("swipeout-overswipe") && (D.css({
								left: (P ? p : 0) + "px"
							}), P ? D.addClass("swipeout-overswipe-active") : D.removeClass("swipeout-overswipe-active")), C.length > 1 && D.css("z-index", C.length - n), D.transform("translate3d(" + (v + p * (1 - Math.min(O, 1))) + "px,0,0)");
				...
				}, O), e ? (i(e).on(r.touchEvents.start, a, O), i(e).on(r.touchEvents.move, t, O), i(e).on(r.touchEvents.end, n, O)) : (i(document).on(r.touchEvents.start, ".list-block li.swipeout", a, O),
					i(document).on(r.touchEvents.move, ".list-block li.swipeout", t, O), i(document).on(r.touchEvents.end, ".list-block li.swipeout", n, O))
					...
					if (0 !== p.length) {
						...
						if (!(T.disabled || e.hasClass("disabled") || k.hasClass("disabled"))) {
							...
							var X = r._compiledTemplates.smartSelectItem,
								j = "";
							if (!w)
								for (var G = 0; G < H.length; G++) j += X(H[G]);
							var _, U, Q = "",
								J = "",
								K = "",
								Z = "";
							...
							var re, ne, ie = '<form class="searchbar searchbar-init" data-search-list=".smart-select-list-' + R + '" data-search-in=".item-title"><div class="searchbar-input"><input type="search" placeholder="' + $ + '"><a href="#" class="searchbar-clear"></a></div>' + (y ? "" : '<a href="#" class="searchbar-cancel">' + ee + "</a>") + '</form><div class="searchbar-overlay"></div>',
								se = ("picker" !== d && "through" === U ? _ : "") + '<div class="pages">  <div data-page="' + ae + '" data-select-name="' + F + '" class="page smart-select-page ' + J + " " + K + " " + Z + '">' + ("picker" !== d && "fixed" === U ? _ : "") + (te ? ie : "") + '    <div class="page-content">' + ("picker" !== d && "static" === U ? _ : "") + '      <div class="list-block ' + (w ? "virtual-list" : "") + " smart-select-list-" + R + " " + (g ? "theme-" + g : "") + '">        <ul>' + (w ? "" : j) + "        </ul>      </div>    </div>  </div></div>";
			...
			r.virtualList = function(e, a) {
				...
				var S, M, I, E, O, D, L, z, B, N, A, H, R, V, Y, q, F, W, X;
			...
			var b = function(e) {
				function a() {
					var e = !1;
					return p.params.convertToPopover || p.params.onlyInPopover ? (!p.inline && p.params.input && (p.params.onlyInPopover ? e = !0 : r.device.ios ? e = !!r.device.ipad : r.width >= 768 && (e = !1)), e) : e
				}
				...
				return p.setValue = function(e, a) {
					...
					var a, t, r = e || [],
						n = [];
					if (0 === p.cols.length)
						for (a = 0; a < p.params.cols.length; a++) t = p.params.cols[a], void 0 !== t.displayValues && void 0 !== t.values && void 0 !== t.values.indexOf(r[a]) ? n.push(t.displayValues[t.values.indexOf(r[a])]) : n.push(r[a]);
					else
			...
			r.addNotification = function(e) {
				...
				e = i(e);
				var t = e.find(".data-table-header"),
					r = e.find(".data-table-header-selected");
				e.on("change", '.checkbox-cell input[type="checkbox"]', function(t) {
					if (!t.detail || !t.detail._sentByF7DataTable) {
						var r = i(this),
							n = r[0].checked;
						r.parents("thead").length > 0 ? e.find("tbody tr")[n ? "addClass" : "removeClass"]("data-table-row-selected").find("input").prop("checked", n).trigger("change", {
							_sentByF7DataTable: !0
						}) : (r.parents("tr")[n ? "addClass" : "removeClass"]("data-table-row-selected"), n ? e.find('tbody .checkbox-cell input[type="checkbox"]:checked').length === e.find("tbody tr").length && e.find('thead .checkbox-cell input[type="checkbox"]').prop("checked", !0).trigger("change", {
							_sentByF7DataTable: !0
						}) : e.find('thead .checkbox-cell input[type="checkbox"]').prop("checked", !1)), a()
					}
			...
			if (void 0 !== g && "undefined" != typeof Dom7 && (g = Dom7), (void 0 !== g || (g = "undefined" == typeof Dom7 ? window.Dom7 || window.Zepto || window.jQuery : Dom7)) && (f.$ = g, f.currentBreakpoint = void 0, f.getActiveBreakpoint = function() {
				...
				f.onTouchStart = function(e) {
					...
					if (e.originalEvent && (e = e.originalEvent), !D || "mousemove" !== e.type) {
						...
						if (D && f.params.touchReleaseOnEdges && !f.params.loop)
							if (f.isHorizontal()) {
